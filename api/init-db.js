/**
 * API Vercel: crea automaticamente la tabella venues_cloud in Supabase.
 * Chiamata dall'app quando rileva che la tabella non esiste.
 * Richiede SUPABASE_DB_URL (connection string da Supabase → Settings → Database).
 */
...
export default async function handler(req, res) {
  ...
  if (!url) {
    return res.status(503).json({
      ok: false,
      error: "SUPABASE_DB_URL non configurato. Aggiungilo in Vercel → Environment Variables.",
    });
  }
  let client;
  try {
