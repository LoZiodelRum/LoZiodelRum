...
export default function PannelloControllo() {
  ...
  const [wineTableName, setWineTableName] = useState("vini");
  ...
  const [createRoleHint, setCreateRoleHint] = useState(null);
  ...
  async function loadData() {
    ...
    const normalizeRole = (value) => String(value || "utente").trim().toLowerCase();
    const normalizeApproved = (user) => {
    ...
    const safeUsers = (Array.isArray(utentiData) ? utentiData : []).map((user) => ({
      ...user,
  ...
  async function toggleApprovazione(user) {
    await supabase
      .from("Profili")
                    ...
                    return (
                      ...
                      <div style={fieldStyle}>
                        ...
                        {booleanFields.has(key) ? (
                          <select value={String(toBoolean(selectedItem[key]))} onChange={(e) => { const value = e.target.value === "true"; setSelectedItem((prev) => ({ ...prev, [key]: value })); setSaveStatus(null); }} style={selectStyle}><option value="true">true</option><option value="false">false</option></select>
                              ...
                              >
                                {(() => {
                                  const selectedValues = typeof selectedItem[key] === "string"
                                    ? selectedItem[key].split(",").map((v) => v.trim()).filter(Boolean)
                                    : [];
