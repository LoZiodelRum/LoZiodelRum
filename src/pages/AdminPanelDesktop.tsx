...
const ADMIN_LOCALI_COLUMNS = "id, nome, indirizzo, citta, provincia, paese, latitudine, longitudine, categoria, orari, price_range, telefono, sito, instagram, image_url, image, video_url, descrizione, descrizione_completa, qualita_drink, competenza_staff, atmosfera, qualita_prezzo, verificato, in_evidenza, status, approvato, created_at, updated_at";
const ADMIN_PROFILI_COLUMNS = "id, nome, cognome, username, email, telefono, ruolo, status, approvato, bio_breve, avatar_url, city, paese, genere, distillato_preferito, cocktail_preferito, intensita_preferita, profilo_gustativo_preferito, famiglia_aromatica_preferita, metodo_consumo_preferito, level, points, badges, numero_recensioni, numero_locali_visitati, numero_cocktail_creati, recensioni, cocktail_creati, locali_segnalati, preferiti, instagram, tiktok, sito_web, nome_locale, esperienza_anni, specialita, certificazioni, menu_caricato, indirizzo_locale, citta_locale, partita_iva, numero_dipendenti, descrizione_locale, created_at, updated_at, ultimo_accesso, email_verificata";
const ADMIN_ARTICOLI_COLUMNS = "id, slug, titolo, titolo_en, titolo_de, titolo_bg, sottotitolo, sottotitolo_en, sottotitolo_de, sottotitolo_bg, estratto, descrizione, contenuto, contenuto_en, contenuto_de, contenuto_bg, immagine, pubblicato, seo_title, seo_title_en, seo_title_de, seo_title_bg, seo_description, seo_description_en, seo_description_de, seo_description_bg, created_at, updated_at";
const ADMIN_COCKTAIL_COLUMNS = "id, nome, nome_en, nome_de, nome_bg, categoria, descrizione, descrizione_en, descrizione_de, descrizione_bg, ingredienti, ingredienti_en, ingredienti_de, ingredienti_bg, preparazione, preparazione_en, preparazione_de, preparazione_bg, immagine, immagine_url, created_at, updated_at";
const ADMIN_DISTILLATI_COLUMNS = "id, nome, nome_en, nome_de, nome_bg, categoria, marca, provenienza, provenienza_en, provenienza_de, provenienza_bg, storia, storia_en, storia_de, storia_bg, note_degustazione, note_degustazione_en, note_degustazione_de, note_degustazione_bg, immagine, created_at, updated_at";
const ADMIN_VINI_COLUMNS = "id, nome, nome_en, nome_de, nome_bg, categoria, categoria_en, categoria_de, categoria_bg, annata, cantina, vitigno, grado_alcolico, zona, denominazione, immagine, note_degustazione, note_degustazione_en, note_degustazione_de, note_degustazione_bg, provenienza, provenienza_en, provenienza_de, provenienza_bg, descrizione, descrizione_en, descrizione_de, descrizione_bg, created_at, updated_at";
...
function getAdminSelectColumns(tableName: string) {
  ...
  if (normalized === "locali") return ADMIN_LOCALI_COLUMNS;
  if (normalized === "profili") return ADMIN_PROFILI_COLUMNS;
  if (normalized === "articoli") return ADMIN_ARTICOLI_COLUMNS;
  if (normalized === "cocktail") return ADMIN_COCKTAIL_COLUMNS;
  if (normalized === "distillati") return ADMIN_DISTILLATI_COLUMNS;
  if (normalized === "vini") return ADMIN_VINI_COLUMNS;
  return "id";
}
...
export default function AdminPanel() {
  ...
  async function loadData() {
    const { data: localiData } = await supabase.from("Locali").select(ADMIN_LOCALI_COLUMNS);
    let adminPassword = "";
  ...
  async function salvaModifiche() {
    ...
    if (!isCreating && !isProfiliTable) {
      let refreshQuery = supabase
        .from(selectedTable)
        .select(getAdminSelectColumns(selectedTable));
      ...
      <div style={contentStyle}>
        ...
        <div style={quickActionGridStyle}>
          ...
          <div style={quickActionCardStyle}>
            ...
            <button style={quickActionBtnStyle} onClick={() => openCreateEditor("articoli", articoli)}>
              modifica articoli
            </button>
          </div>
        </div>
