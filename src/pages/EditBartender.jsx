...
import { uploadMultipleToSupabaseStorage, urlsToDbString, dbStringToUrls } from "@/lib/supabaseStorage";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (photoFiles.length > 0) {
        ...
        const existingUrls = dbStringToUrls(formData.photo);
        photoUrl = urlsToDbString([...existingUrls, ...urls]);
      }
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    accept="image/*,video/*"
                    capture="environment"
                    id="edit-bartender-photo-camera"
                    onChange={(e) => {
                  ...
                  <div className="flex gap-2 mt-1">
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => document.getElementById("edit-bartender-photo-camera")?.click()}
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => document.getElementById("edit-bartender-photo-gallery")?.click()}
                      className="bg-stone-800 border-stone-600 text-stone-300 hover:bg-stone-700"
                    >
                      Galleria
                    </Button>
                  ...
                  {(formData.photo || photoFiles.length > 0) && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {dbStringToUrls(formData.photo).map((url, i) => (
                        <div key={`url-${i}`} className="relative">
                          <img src={url} alt="" className="h-20 w-20 object-cover rounded-xl" onError={(e) => e.target.style.display = "none"} />
                        </div>
                      ))}
                      {photoFiles.map((f, i) => (
